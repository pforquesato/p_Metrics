...
class p_OLS_raw(object):
    ...
    def _p_OlsRun(self, y, X, seMethod, clusterDummies):
        ...
        if clusterDummies is not None and seMethod is 'robust':                
            ...
            varCoVar = p_Bootstrap.p_Bootstrap(W, bCluster=clusterDummies)
...
class p_OLS(p_OLS_raw):
    ...
    def _p_Dummify(self, dataFrame, xVariable, yVariable, cluster):
        ...
        # Interactions are still being implemented...
        for varIndex, varName in enumerate(xVariable):
            if '*' in varName:
                # So varName is an interaction
                # First get the underlying variables
                splittedVar = varName.split('*')
                for splitIndx in range(len(splittedVar)):
                    splittedVar[splitIndx] = splittedVar[splitIndx].replace(' ', '')
                
                # Now before we start, we make sure all variables are in
                # xVariable, as it is not kosher to use interactions without
                # the base variable.
                for splitIndx in range(len(splittedVar)):                                
                    if splittedVar[splitIndx] not in xVariable:
                        xVariable.append(splittedVar[splitIndx])
                
                # OK. So now, is any of them a dummy? If not, it is easier.
                if not any([x.startswith('factor:') for x in splittedVar]):            
                    splittedLen = len(splittedVar) # Get No. of interacted variables.
                    # And then just multiply the variables.
                    dataFrame[varName] = dataFrame.loc[:, splittedVar[0]]
                    for i in range(1, splittedLen):
                        dataFrame[varName] = np.multiply(dataFrame[varName], 
                            dataFrame.loc[:, splittedVar[i]])
                ...
                else: raise NotImplementedError('Sorry!')
        '''
...
if __name__ == '__main__':
    ...
    # Create series-clustered errors
    ...
    # We can run the OLS...
    testReg1 = p_OLS(dataFrameTest, 'y', ['x2', 'x3'], autoPrint=False)
    
    # ... try it without intercept...
    testReg2 = p_OLS(dataFrameTest, 'y', ['x2', 'x3'], 
        intercept=False, autoPrint=False)
    
    # ... add factors...
    testReg3 = p_OLS(dataFrameTest, 'y', ['x2', 'x3', 'Series'], 
        autoPrint=False)
    ...
    # ... or even use bootstrap.
    testReg6 = p_OLS(dataFrameTest, 'y', ['x2', 'x3'], 
        seMethod='bootstrap', autoPrint=False)
    
    # We can use clustered SE, either with Heteroskedasticity
    # robust...
    testReg7 = p_OLS(dataFrameTest, 'y', ['x2', 'x3'], 
        cluster='Series', autoPrint=False)
    
    # ... or bootstrap.
    testReg8 = p_OLS(dataFrameTest, 'y', ['x2', 'x3'], 
        seMethod='bootstrap', cluster='Series', 
        autoPrint=False)
